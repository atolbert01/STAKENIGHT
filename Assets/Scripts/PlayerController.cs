...
public class PlayerController : MonoBehaviour
{
    ...
    private bool _jumpNotHeld = false;
    private void Update()
    {
        ...
        if (_canControl)
        {
            ...
            if (!_jumpNotHeld) _jumpNotHeld = !Input.GetKey(KeyCode.Space);
            if (_jumpNotHeld && isGrounded && _jumpEnabled && Input.GetKeyDown(KeyCode.Space) && !_isGonnaJump)
            {
                _isGonnaJump = true;
                _jumpNotHeld = false;
            }
    ...
    private bool IsGrounded(out Vector3 groundNormal)
    {
        ...
        foreach (Transform t in _groundedRayPositions)
        {
            ...
            bool hitGround = Physics.Raycast(t.position, -groundNormal, out RaycastHit hit, 0.51f, ~_excludedLayers);
            if (hitGround)
            {
